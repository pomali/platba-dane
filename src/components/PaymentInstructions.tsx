...
const CREDITOR_NAME = "Financne riaditelstvo Slovenskej republiky";
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1 py-3 border-b border-gray-100 last:border-0">
      ...
      <dd className="flex items-center gap-1 flex-wrap">
        ...
        {note && (
          <span className="text-xs text-gray-400 w-full mt-0.5">{note}</span>
        )}
      </dd>
    </div>
  );
...
export function PaymentInstructions({ taxData }: PaymentInstructionsProps) {
  ...
  const selectedTaxType =
    TAX_TYPES.find((t) => t.code === taxTypeCode) || TAX_TYPES[0];
  ...
  const payment = isReady
    ...
    : null;
  const isIbanValid = payment ? isValidIBAN(payment.iban) : false;
  ...
  useEffect(() => {
    ...
    const sepaQr = [
      ...
      CREDITOR_NAME, // Beneficiary name (optional)
      iban, // IBAN
      `EUR${suma.toFixed(2)}`, // Amount
      "",
      `/VS${payment.variabilnySymbol}/SS/KS`, // Remittance info
      "Platba dane z prijmov FO", // Additional info
    ].join("\n");
    ...
    const paymeParams = new URLSearchParams({
      ...
      MSG: "Platba dane z prijmov FO",
      CN: CREDITOR_NAME, // Creditor Name
    });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
        <span className="text-green-600">💳</span> Platobné údaje
      </h2>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
        ...
        <p className="text-xs text-amber-700 mb-2">
          OÚD nájdete v online zóne Finančnej správy (
          <a
        ...
        <p className="text-xs text-amber-700 mb-2">
          Zadajte iba čísla, bez medzier a lomiek.
        </p>
        <input
          type="text"
          value={oud}
      ...
      <div className="mb-6 text-sm text-gray-700 space-y-1">
        <p>
          <span className="font-medium text-gray-800">Druh dane:</span>{" "}
          {taxData.druhDane ?? selectedTaxType.name}
        </p>
        <p>
          <span className="font-medium text-gray-800">Typ úhrady:</span> Daňové
          priznanie (kód 00)
        </p>
        <p className="text-xs text-gray-400">
          Predčíslie účtu: {selectedTaxType.prefix} / Kód dane:{" "}
          {selectedTaxType.vsPrefix}
        </p>
      </div>
      ...
      {payment ? (
        <div>
          <dl className="bg-gray-50 rounded-lg px-4 py-2">
            <InfoRow
              ...
              note={`${isIbanValid ? "Číslo účtu" : "Neplatný IBAN, skontrolujte údaje"}: ${payment.accountFormatted}`}
            />
            <InfoRow
            ...
            <InfoRow
              ...
              note={`Skladba: kód dane ${selectedTaxType.vsPrefix} + typ úhrady 00 + rok ${rok}`}
            />
            <InfoRow label="Suma" value={`${payment.suma.toFixed(2)} €`} />
            <InfoRow label="Zdaňovacie obdobie" value={rok} />
            <InfoRow
          ...
          {(qrDataUrl || spaydQrDataUrl || paymeQrDataUrl) && (
            <div className="mt-6 flex flex-col sm:flex-row justify-center gap-8">
              {spaydQrDataUrl && (
                <div className="flex flex-col items-center gap-2">
                  <p className="text-sm font-medium text-gray-600">
                    QR pre SK/CZ bankové aplikácie (SPAYD)
                  </p>
                  <img
                    src={spaydQrDataUrl}
                  ...
                  />
                  <p className="text-xs text-gray-400">
                    Najčastejšie funguje v slovenských a českých bankových
                    aplikáciách
                  </p>
                </div>
              )}
              {paymeQrDataUrl && (
                <div className="flex flex-col items-center gap-2">
                  <p className="text-sm font-medium text-gray-600">Payme.sk</p>
                  <img
                    src={paymeQrDataUrl}
                  ...
                  {paymeUrl && (
                    ...
                    >
                      Otvoriť platbu cez payme.sk
                    </a>
                  )}
                  <p className="text-xs text-gray-400">
                    Alternatívny spôsob otvorenia predvyplnenej platby
                  </p>
                </div>
              )}
              {qrDataUrl && (
                <div className="flex flex-col items-center gap-2">
                  ...
                  />
                  <p className="text-xs text-gray-400">
                    Štandard SEPA Credit Transfer, podpora závisí od banky
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
            <strong>Pred odoslaním platby:</strong> skontrolujte IBAN,
            variabilný symbol a sumu. Pri nejasnostiach kontaktujte váš daňový
            úrad alebo využite{" "}
            <a
        ...
        <div className="text-center py-8 text-gray-400">
          <p className="text-sm">
            {oud.trim().length === 0
              ? "Zadajte OÚD na zobrazenie platobných údajov."
              : oud.trim().length < 8
                ? "OÚD musí mať aspoň 8 číslic."
                : suma <= 0
                  ? "Doplňte sumu dane na úhradu."
